...
#以AES-128为例
#AES分组加密块大小为16个字节，故data需要为16个字节否则填充
#AES的加密key必须为16个字节，IV向量必须为16个字节
...
def encrypt_ECB(key,data):
    ...
    cipher = AES.new(key=real_key,mode=AES.MODE_ECB)
    padding_data = pad(real_data,AES.block_size)
...
def encrypt_CBC(key,data,iv):
    ...
    cipher = AES.new(key=real_key,iv=real_iv,mode=AES.MODE_CBC)
    real_data = pad(real_data,AES.block_size)
